...
TEST(DeleteStatementTest) {
  ...
  ASSERT_EQ(result.statements_size(), 1);
  ASSERT(result.statement(0)->type() == kStmtDelete);

  const DeleteStatement* stmt = (const DeleteStatement*)result.statement(0);
...
TEST(CreateStatementTest) {
  ...
  ASSERT(result.is_valid());
  ASSERT_EQ(result.statements_size(), 1);
  ASSERT_EQ(result.statement(0)->type(), kStmtCreate);

  const CreateStatement* stmt = (const CreateStatement*)result.statement(0);
  ...
  ASSERT_NEQ(stmt->columns->at(6)->type,
             (ColumnType{DataType::DECIMAL, 0, 14}));
  ASSERT_NEQ(stmt->columns->at(6)->type,
             (ColumnType{DataType::DECIMAL, 1, 13}));
  ASSERT_EQ(stmt->columns->at(6)->nullable, true);
  // c_decimal_precision_scale DECIMAL(13,37)
  ASSERT_STREQ(stmt->columns->at(7)->name, "c_decimal_precision_scale");
  ASSERT_EQ(stmt->columns->at(7)->type,
            (ColumnType{DataType::DECIMAL, 0, 13, 37}));
  ASSERT_NEQ(stmt->columns->at(7)->type,
             (ColumnType{DataType::DECIMAL, 0, 14, 37}));
  ASSERT_NEQ(stmt->columns->at(7)->type,
             (ColumnType{DataType::DECIMAL, 0, 13, 38}));
  ASSERT_NEQ(stmt->columns->at(7)->type,
             (ColumnType{DataType::DECIMAL, 1, 13, 37}));
  ...
  ASSERT(
      stmt->columns->at(8)->column_constraints->count(ConstraintType::NotNull));
  // c_float FLOAT
  ...
  ASSERT(
      stmt->columns->at(11)->column_constraints->count(ConstraintType::Null));
  // c_long LONG
  ...
  // Expecting two elements in column_constraints since information about NULL
  // constraints is separately stored in ColumnDefinition::nullable
  ASSERT_EQ(stmt->columns->at(15)->column_constraints->size(), 3);
  ASSERT(
      stmt->columns->at(15)->column_constraints->count(ConstraintType::Unique));
  ASSERT(stmt->columns->at(15)->column_constraints->count(
  ...
  // Table constraints are identified and separated during the parsing of the
  // SQL string Table constraints:
  // - PRIMARY KEY(c_char, c_int)
...
TEST(CreateAsSelectStatementTest) {
  SQLParserResult result;
  SQLParser::Parse(
      "CREATE TABLE students_2 AS SELECT student_number, grade FROM students",
      &result);
...
TEST(UpdateStatementTest) {
  SQLParserResult result;
  SQLParser::Parse(
      "UPDATE students SET grade = 5.0, name = 'test' WHERE name = 'Max "
      "O''Mustermann';",
      &result);

  ASSERT(result.is_valid());
  ASSERT_EQ(result.statements_size(), 1);
  ASSERT_EQ(result.statement(0)->type(), kStmtUpdate);

  const UpdateStatement* stmt = (const UpdateStatement*)result.statement(0);
...
TEST(InsertStatementTest) {
  TEST_PARSE_SINGLE_SQL(
      "INSERT INTO students VALUES ('Max Mustermann', 12345, 'Musterhausen', "
      "2.0)",
...
TEST(AlterStatementDropActionTest) {
  SQLParserResult result;
  SQLParser::Parse("ALTER TABLE mytable DROP COLUMN IF EXISTS mycolumn",
                   &result);

  ASSERT(result.is_valid());
  ASSERT_EQ(result.statements_size(), 1);

  const AlterStatement* stmt = (const AlterStatement*)result.statement(0);
  ASSERT_STREQ(stmt->name, "mytable");
  ASSERT_EQ(stmt->ifTableExists, false);

  auto dropAction = (const DropColumnAction*)stmt->action;

  ASSERT_EQ(dropAction->type, msql::ActionType::DropColumn);
...
TEST(CreateIndexStatementIfNotExistsTest) {
  SQLParserResult result;
  SQLParser::Parse(
      "CREATE INDEX IF NOT EXISTS myindex ON myTable (col1, col2);", &result);
...
TEST(DropIndexTest) {
  ...
  const DropStatement* stmt = (const DropStatement*)result.statement(0);
  ASSERT_STREQ(stmt->indexName, "myindex");
...
TEST(DropTableStatementTest) {
  TEST_PARSE_SINGLE_SQL("DROP TABLE students", kStmtDrop, DropStatement, result,
                        stmt);
...
TEST(DropTableIfExistsStatementTest) {
  TEST_PARSE_SINGLE_SQL("DROP TABLE IF EXISTS students", kStmtDrop,
                        DropStatement, result, stmt);
...
TEST(ShowColumnsStatementTest) {
  TEST_PARSE_SINGLE_SQL("SHOW COLUMNS students;", kStmtShow, ShowStatement,
                        result, stmt);
...
TEST(DescribeStatementTest) {
  TEST_PARSE_SINGLE_SQL("DESCRIBE students;", kStmtShow, ShowStatement, result,
                        stmt);
...
TEST(ImportStatementTest) {
  TEST_PARSE_SINGLE_SQL("IMPORT FROM TBL FILE 'students_file' INTO students;",
                        kStmtImport, ImportStatement, result, stmt);
...
TEST(CopyStatementTest) {
  TEST_PARSE_SINGLE_SQL("COPY students FROM 'students_file' ;", kStmtImport,
                        ImportStatement, import_result, import_stmt);
  ...
  TEST_PARSE_SINGLE_SQL("COPY students TO 'students_file';", kStmtExport,
                        ExportStatement, export_result, export_stmt);
...
TEST(MoveSQLResultTest) {
  ...
  ASSERT(res.is_valid());
  ASSERT_EQ(1, res.statements_size());
  ...
  ASSERT_EQ(0, res.statements_size());

  ASSERT(new_res.is_valid());
  ASSERT_EQ(1, new_res.statements_size());
}
...
TEST(StringLengthTest) {
  ...
  ASSERT_EQ(result.statement(0)->stringLength, 18);
  ASSERT_EQ(result.statement(1)->stringLength, 28);
  ASSERT_EQ(result.statement(2)->stringLength, 21);
}

TEST(ExceptOperatorTest) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students EXCEPT SELECT * FROM students_2;", kStmtSelect,
      SelectStatement, result, stmt);

  ASSERT_STREQ(
      stmt->setOperations->back()->nestedSelectStatement->fromTable->name,
      "students_2");
...
TEST(IntersectOperatorTest) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students INTERSECT SELECT * FROM students_2;", kStmtSelect,
      SelectStatement, result, stmt);
...
TEST(UnionOperatorTest) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students UNION SELECT * FROM students_2;", kStmtSelect,
      SelectStatement, result, stmt);
...
TEST(UnionAllOperatorTest) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students UNION ALL SELECT * FROM students_2;", kStmtSelect,
      SelectStatement, result, stmt);
...
TEST(NestedSetOperationTest) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students INTERSECT SELECT grade FROM students_2 UNION "
...
TEST(OrderByFullStatementTest) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM students INTERSECT SELECT grade FROM students_2 UNION "
      "SELECT * FROM employees ORDER BY grade ASC;",
      kStmtSelect, SelectStatement, result, stmt);

  ASSERT_EQ(stmt->setOperations->back()->resultOrder->at(0)->type, kOrderAsc);
  ASSERT_STREQ(stmt->setOperations->back()->resultOrder->at(0)->expr->name,
               "grade");
  ASSERT_FALSE(stmt->setOperations->back()->isAll);
}
...
TEST(SetOperationSubQueryOrder) {
  TEST_PARSE_SINGLE_SQL(
      "(SELECT * FROM students ORDER BY name DESC) INTERSECT SELECT grade FROM "
      "students_2 UNION SELECT * FROM "
...
TEST(NestedDifferentSetOperationsWithWithClause) {
  TEST_PARSE_SINGLE_SQL(
      "WITH UNION_FIRST AS (SELECT * FROM A UNION SELECT * FROM B) SELECT * "
      "FROM UNION_FIRST EXCEPT SELECT * FROM C",
      kStmtSelect, SelectStatement, result, stmt);

  ASSERT_STREQ(stmt->withDescriptions->back()->alias, "UNION_FIRST");
  ASSERT_EQ(
      stmt->withDescriptions->back()->select->setOperations->back()->setType,
      kSetUnion);
  ASSERT_STREQ(stmt->withDescriptions->back()->select->fromTable->name, "A");
  ASSERT_STREQ(stmt->withDescriptions->back()
                   ->select->setOperations->back()
                   ->nestedSelectStatement->fromTable->name,
               "B");
  ...
  ASSERT_STREQ(
      stmt->setOperations->back()->nestedSelectStatement->fromTable->name, "C");
}

TEST(NestedAllSetOperationsWithWithClause) {
  TEST_PARSE_SINGLE_SQL(
      "WITH UNION_FIRST AS (SELECT * FROM A UNION SELECT * FROM B) SELECT * "
      "FROM UNION_FIRST EXCEPT SELECT * FROM "
  ...
  ASSERT_STREQ(stmt->fromTable->name, "UNION_FIRST");
  ASSERT_EQ(
      stmt->setOperations->back()
          ->nestedSelectStatement->fromTable->select->setOperations->back()
          ->setType,
      kSetIntersect);
  ASSERT_STREQ(stmt->setOperations->back()
                   ->nestedSelectStatement->fromTable->select->fromTable->name,
               "C");
  ASSERT_STREQ(
      stmt->setOperations->back()
          ->nestedSelectStatement->fromTable->select->setOperations->back()
...
TEST(NestedSetOperationsWithMultipleWithClauses) {
  TEST_PARSE_SINGLE_SQL(
      "WITH UNION_FIRST AS (SELECT * FROM A UNION SELECT * FROM "
      "B),INTERSECT_SECOND AS (SELECT * FROM UNION_FIRST "
      "INTERSECT SELECT * FROM C) SELECT * FROM UNION_FIRST EXCEPT SELECT * "
      "FROM INTERSECT_SECOND",
      kStmtSelect, SelectStatement, result, stmt);
  ...
  ASSERT_EQ(
      stmt->withDescriptions->at(0)->select->setOperations->back()->setType,
      kSetUnion);
  ASSERT_STREQ(stmt->withDescriptions->at(0)->select->fromTable->name, "A");
  ASSERT_STREQ(stmt->withDescriptions->at(0)
                   ->select->setOperations->back()
                   ->nestedSelectStatement->fromTable->name,
               "B");
  ...
  ASSERT_EQ(
      stmt->withDescriptions->back()->select->setOperations->back()->setType,
      kSetIntersect);
  ASSERT_STREQ(stmt->withDescriptions->back()->select->fromTable->name,
               "UNION_FIRST");
  ASSERT_STREQ(stmt->withDescriptions->back()
                   ->select->setOperations->back()
                   ->nestedSelectStatement->fromTable->name,
...
TEST(WrongOrderByStatementTest) {
  SQLParserResult res = parse_and_move(
      "SELECT * FROM students ORDER BY name INTERSECT SELECT grade FROM "
      "students_2;");
  ASSERT_FALSE(res.is_valid());
}
...
TEST(BeginTransactionTest) {
  {
    TEST_PARSE_SINGLE_SQL("BEGIN TRANSACTION;", kStmtTransaction,
                          TransactionStatement, transaction_result,
                          transaction_stmt);
  ...
  {
    TEST_PARSE_SINGLE_SQL("BEGIN;", kStmtTransaction, TransactionStatement,
                          transaction_result, transaction_stmt);
...
TEST(RollbackTransactionTest) {
  TEST_PARSE_SINGLE_SQL("ROLLBACK TRANSACTION;", kStmtTransaction,
                        TransactionStatement, transaction_result,
                        transaction_stmt);
...
TEST(CommitTransactionTest) {
  TEST_PARSE_SINGLE_SQL("COMMIT TRANSACTION;", kStmtTransaction,
                        TransactionStatement, transaction_result,
                        transaction_stmt);
...
TEST(CastAsType) {
  TEST_PARSE_SINGLE_SQL("SELECT CAST(ID AS VARCHAR(8)) FROM TEST", kStmtSelect,
                        SelectStatement, result, stmt);
