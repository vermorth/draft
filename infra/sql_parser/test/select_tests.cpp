...
TEST(SelectTest) {
  TEST_PARSE_SINGLE_SQL("SELECT * FROM students;", kStmtSelect, SelectStatement,
                        result, stmt);
...
TEST(SelectExprTest) {
  TEST_PARSE_SINGLE_SQL("SELECT a, MAX(b), CUSTOM(c, F(un)) FROM students;",
  ...
  ASSERT(stmt->selectList->at(2)->exprList->at(1)->exprList->at(0)->isType(
      kExprColumnRef));
  ASSERT_STREQ(
      stmt->selectList->at(2)->exprList->at(1)->exprList->at(0)->getName(),
      "un");
}

TEST(SelectUnaryMinusTest) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT 10 - 20, 10 + -20, 10 +-20, 10+-20, 9223372036854775807, "
      "-9223372036854775808, 10-5.2, 10+-5.2",
      kStmtSelect, SelectStatement, result, stmt);
...
TEST(SelectSubstrTest) {
  TEST_PARSE_SINGLE_SQL("SELECT SUBSTR(a, 3, 5) FROM students;", kStmtSelect,
...
TEST(SelectHavingTest) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT city, AVG(grade) AS avg_grade FROM students GROUP BY city HAVING "
      "AVG(grade) < -2.0",
      kStmtSelect, SelectStatement, result, stmt);
...
TEST(SelectDistinctTest) {
  TEST_PARSE_SINGLE_SQL("SELECT DISTINCT grade, city FROM students;",
                        kStmtSelect, SelectStatement, result, stmt);
...
TEST(SelectSchemaTest) {
  TEST_PARSE_SINGLE_SQL("SELECT grade FROM some_schema.students;", kStmtSelect,
                        SelectStatement, result, stmt);
...
TEST(SelectGroupDistinctTest) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT city, COUNT(name), COUNT(DISTINCT grade) FROM students GROUP BY "
      "city;",
      kStmtSelect, SelectStatement, result, stmt);
...
TEST(SelectConditionalSelectTest) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT * FROM t WHERE a = (SELECT MIN(v) FROM tt) AND EXISTS (SELECT * "
      "FROM test WHERE x < a);",
      kStmtSelect, SelectStatement, result, stmt);
...
TEST(SelectCaseValueWhenWhenElse) {
  TEST_PARSE_SINGLE_SQL(
      "SELECT CASE x WHEN 1 THEN 0 WHEN 2 THEN 3 WHEN 8 THEN 7 ELSE 4 END FROM "
      "test;",
      kStmtSelect, SelectStatement, result, stmt);
...
TEST(SelectAliasSimple) {
  TEST_PARSE_SINGLE_SQL("SELECT * FROM students AS s1;", kStmtSelect,
                        SelectStatement, result, stmt);
...
TEST(SelectAliasWithColumns) {
  TEST_PARSE_SINGLE_SQL("SELECT * FROM students AS s1(id, city);", kStmtSelect,
                        SelectStatement, result, stmt);
...
TEST(Operators) {
  ...
  stmt = (SelectStatement*)result.statement(0);
  ...
  stmt = (SelectStatement*)result.statement(1);
  ASSERT_EQ(stmt->whereClause->opType, kOpEquals);
  ASSERT_EQ(stmt->whereClause->expr2->ival, 2);

  stmt = (SelectStatement*)result.statement(2);
  ASSERT_EQ(stmt->whereClause->opType, kOpNotEquals);

  stmt = (SelectStatement*)result.statement(3);
  ASSERT_EQ(stmt->whereClause->opType, kOpNotEquals);

  stmt = (SelectStatement*)result.statement(4);
  ASSERT_EQ(stmt->whereClause->opType, kOpGreater);

  stmt = (SelectStatement*)result.statement(5);
  ASSERT_EQ(stmt->whereClause->opType, kOpLess);

  stmt = (SelectStatement*)result.statement(6);
  ASSERT_EQ(stmt->whereClause->opType, kOpGreaterEq);

  stmt = (SelectStatement*)result.statement(7);
  ASSERT_EQ(stmt->whereClause->opType, kOpLessEq);

  stmt = (SelectStatement*)result.statement(8);
  ASSERT_EQ(stmt->whereClause->opType, kOpEquals);
  ASSERT_EQ(stmt->whereClause->expr2->ival, 1);
  ASSERT_EQ(stmt->whereClause->expr2->isBoolLiteral, true);

  stmt = (SelectStatement*)result.statement(9);
  ASSERT_EQ(stmt->whereClause->opType, kOpEquals);
...
TEST(JoinTypes) {
  ...
  stmt = (SelectStatement*)result.statement(10);
  ASSERT_EQ(stmt->fromTable->join->type, kJoinCross);

  stmt = (SelectStatement*)result.statement(11);
...
TEST(SetLimitOffset) {
  ...
  stmt = (SelectStatement*)result.statement(12);
  ASSERT_EQ(stmt->limit->limit->type, kExprLiteralInt);
  ASSERT_EQ(stmt->limit->limit->ival, 10);
  ASSERT_NULL(stmt->limit->offset);

  stmt = (SelectStatement*)result.statement(13);
...
TEST(CastExpression) {
  TEST_PARSE_SINGLE_SQL("SELECT CAST(10 AS INT);", kStmtSelect, SelectStatement,
                        result, stmt);
...
TEST(NoFromClause) {
  TEST_PARSE_SINGLE_SQL("SELECT 1 + 2;", kStmtSelect, SelectStatement, result,
                        stmt);
...
TEST(WithClauseSingle) {
  ...
  ASSERT_STREQ(stmt->withDescriptions->at(0)->select->selectList->at(0)->name,
               std::string("name"));
  ASSERT_STREQ(stmt->withDescriptions->at(0)->select->fromTable->name,
               std::string("peopleA"));
...
TEST(WithClauseDouble) {
  ...
  ASSERT_STREQ(stmt->withDescriptions->at(1)->select->fromTable->name,
               "peopleB");
...
TEST(CastAsDate) {
  TEST_PARSE_SINGLE_SQL("SELECT CAST(ID AS DATE) FROM TEST", kStmtSelect,
                        SelectStatement, result, stmt);
...
TEST(DateLiteral) {
  TEST_PARSE_SINGLE_SQL("SELECT * FROM t WHERE a = DATE '1996-12-31'",
                        kStmtSelect, SelectStatement, result, stmt);
  ASSERT_TRUE(result.is_valid());
  stmt = (SelectStatement*)result.statement(0);
  ASSERT_EQ(stmt->whereClause->opType, kOpEquals);
...
TEST(IntervalLiteral) {
  ...
  const auto interval_units = std::map<DatetimeField, std::string>{
      {kDatetimeSecond, "second"}, {kDatetimeMinute, "minute"},
      {kDatetimeHour, "hour"},     {kDatetimeDay, "day"},
      {kDatetimeMonth, "month"},   {kDatetimeYear, "year"}};
...
TEST(MultipleLockingClause) {
  SelectStatement* stmt;
  TEST_PARSE_SQL_QUERY(
      "SELECT * FROM t, c WHERE t.a = 10 FOR NO KEY UPDATE OF t FOR KEY SHARE "
      "OF c;"
      "SELECT * FROM t, c WHERE t.a = 10 FOR SHARE OF t SKIP LOCKED FOR UPDATE "
      "OF c NOWAIT;"
      "SELECT * FROM t, c, s WHERE t.a = 10 FOR NO KEY UPDATE FOR SHARE OF t "
      "SKIP LOCKED FOR UPDATE OF c, s NOWAIT;",
      result, 3);
  ...
  ASSERT_EQ(stmt->lockings->at(1)->rowLockWaitPolicy,
            RowLockWaitPolicy::NoWait);
  ASSERT_EQ(stmt->lockings->at(0)->tables->size(), 1);
  ASSERT_STREQ(stmt->lockings->at(1)->tables->at(0), "c");
  ...
  ASSERT_EQ(stmt->lockings->at(2)->rowLockWaitPolicy,
            RowLockWaitPolicy::NoWait);
