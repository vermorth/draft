...
TEST(AutoQueryFileTest) {
  ...
  if (num_failed == 0) {
    printf(
        "\033[0;32m{      ok} \033[0mAll %lu grammar tests completed "
        "successfully!\n",
        num_executed);
  } else {
    fprintf(stderr,
            "\033[0;31m{  failed} \033[0mSome grammar tests failed! %lu out of "
            "%lu tests failed!\n",
