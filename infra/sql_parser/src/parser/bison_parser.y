...
#include <string.h>
  ...
  int yyerror(YYLTYPE * llocp, SQLParserResult * result, yyscan_t scanner, const char* msg) {
    ...
    result->set_error_details(strdup(msg), llocp->first_line, llocp->first_column);
    return 0;
  }
  // clang-format off
%}
...
%parse-param { msql::SQLParserResult* result }
...
%union {
  ...
  // statements
  msql::AlterStatement* alter_stmt;
  msql::CreateStatement* create_stmt;
  msql::DeleteStatement* delete_stmt;
  msql::DropStatement* drop_stmt;
  msql::ExecuteStatement* exec_stmt;
  msql::ExportStatement* export_stmt;
  msql::ImportStatement* import_stmt;
  msql::InsertStatement* insert_stmt;
  msql::PrepareStatement* prep_stmt;
  msql::SelectStatement* select_stmt;
  msql::ShowStatement* show_stmt;
  msql::SQLStatement* statement;
  msql::TransactionStatement* transaction_stmt;
  msql::UpdateStatement* update_stmt;

  msql::Alias* alias_t;
  msql::AlterAction* alter_action_t;
  msql::ColumnDefinition* column_t;
  msql::ColumnType column_type_t;
  msql::ConstraintType column_constraint_t;
  msql::DatetimeField datetime_field;
  msql::DropColumnAction* drop_action_t;
  msql::Expr* expr;
  msql::GroupByDescription* group_t;
  msql::ImportType import_type_t;
  msql::JoinType join_type;
  msql::LimitDescription* limit;
  msql::OrderDescription* order;
  msql::OrderType order_type;
  msql::SetOperation* set_operator_t;
  msql::TableConstraint* table_constraint_t;
  msql::TableElement* table_element_t;
  msql::TableName table_name;
  msql::TableRef* table;
  msql::UpdateClause* update_t;
  msql::WithDescription* with_description_t;
  msql::LockingClause* locking_t;

  std::vector<char*>* str_vec;
  std::unordered_set<msql::ConstraintType>* column_constraint_set;
  std::vector<msql::Expr*>* expr_vec;
  std::vector<msql::OrderDescription*>* order_vec;
  std::vector<msql::SQLStatement*>* stmt_vec;
  std::vector<msql::TableElement*>* table_element_vec;
  std::vector<msql::TableRef*>* table_vec;
  std::vector<msql::UpdateClause*>* update_vec;
  std::vector<msql::WithDescription*>* with_description_vec;
  std::vector<msql::LockingClause*>* locking_clause_vec;

  std::pair<int64_t, int64_t>* ival_pair;

  msql::RowLockMode lock_mode_t;
  msql::RowLockWaitPolicy lock_wait_policy_t;
}
